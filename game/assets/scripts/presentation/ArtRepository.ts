...
import { allCosmetics, DEFAULT_EQUIPPED, type EquippedCosmetics } from '../economy/catalog';
...
export class ArtRepository {
  ...
  private startupFramePaths(equipped: EquippedCosmetics): string[] {
    ...
    const equippedTheme = allCosmetics().find((item) => item.id === equipped.buttonTheme);
    if (equippedTheme?.primaryAsset) paths.add(equippedTheme.primaryAsset);
    if (equippedTheme?.secondaryAsset) paths.add(equippedTheme.secondaryAsset);
    if (equippedTheme?.rewardAsset) paths.add(equippedTheme.rewardAsset);
    if (equippedTheme?.creamAsset) paths.add(equippedTheme.creamAsset);
