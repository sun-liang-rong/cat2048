...
DEFAULT_ROOT = Path(__file__).resolve().parents[1] / "packages" / "game" / "assets" / "resources" / "game"
DEFAULT_QUALITY = 70
...
def _save_png_candidate(source: Path, candidate: Path, quality: int) -> None:
    if Image is None:
        raise RuntimeError("Pillow is required; install dependencies from scripts/requirements.txt")
